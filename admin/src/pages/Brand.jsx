...
const Brand = ({ token }) => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(backendUrl + '/api/brand/add', formData, { headers: { token } })
  ...
  return (
    <div className='flex flex-col gap-10'>
      <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3 bg-white p-5 rounded-lg shadow-sm'>
        <p className='text-lg font-bold'>Add Brand</p>
        <div className='w-full'>
          <p className='mb-2'>Brand Name</p>
          <input onChange={(e) => setName(e.target.value)} value={name} className='w-full max-w-[500px] border rounded-md px-3 py-2' type="text" placeholder="e.g. Dell" required />
        </div>
        <div className='w-full'>
        ...
        <div>
          <p className='mb-2'>Upload Logo (Optional)</p>
          <label htmlFor="image">
      ...
      <div className='bg-white p-5 rounded-lg shadow-sm'>
        <p className='text-lg font-bold mb-5'>Brand List</p>
        <div className='flex flex-col gap-2'>
          ...
          {list.map((item, index) => (
            <div className='grid grid-cols-[1fr_3fr_1fr] md:grid-cols-[1fr_3fr_2fr_1fr] items-center gap-2 py-1 px-2 border text-sm' key={index}>
              ...
              <p onClick={() => removeBrand(item._id)} className='text-right md:text-center cursor-pointer text-lg font-bold text-red-500'>X</p>
            </div>
          ))}
        </div>
      </div>
